...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="h1">
		...
		<xsl:variable name="directory-name" select="fn:create-id(.)"/>
		<xsl:variable name="file-name" select="concat($level-1-element-name, '.xml')"/>
		<xsl:variable name="output-directory" select="concat($output-directory, $directory-name, '/')"/>
		...
		<xsl:result-document href="{concat($output-directory, $file-name)}">
			<xsl:element name="{$level-1-element-name}">
				...
				<xsl:attribute name="xsi:noNamespaceSchemaLocation" select="fn:root-path($output-directory, 'docs.xsd')"/>
				...
				<description>
					<xsl:text>TODO: Add description.</xsl:text>
				</description>
				<introduction>
				...
				<xsl:if test="$level-2">
					<xsl:element name="{$level-2-property-name}">
						<xsl:apply-templates select="$level-2">
							<xsl:with-param name="output-directory" select="$output-directory"/>
	...
	<xsl:template match="h2">
		...
		<!--<xsl:variable name="directory-name" select="fn:create-id(.)"/>
		<xsl:variable name="file-name" select="concat($level-2-element-name, '.xml')"/>
		<xsl:variable name="output-directory" select="concat($output-directory, $directory-name, '/')"/>-->
