...
# Generation Settings
#
# force_overwrite_navigation_yml - will overwrite existing 
#		 aaaa-navigation.yml files, any ordering in the files is overwritten
#		 to an alphabetical one, any content_footer settings are removed
#
# force_overwrite_assets_yml - will overwrite existing
#		 aaaa-assets.yml files, any img_alt, img_caption, and img_tooltips
#		 text is set back to nil/null
#
#######################################################################
...
def process_navigation_name(folder_root)
	...
	SUBS.secondpass.each do |s|
		p.gsub! /#{s.original}/, s.target
	end

	# lowercase
	SUBS.lowercase.split(" ").each do |w|
...
####################################################################################################
# Traverse Folder Structure of Source Content, Create Output Structure, Copy Images Folders
####################################################################################################

# Will add this method to routine
def traverse_and_create_output_structure(node_folder_path)
...
nav_tree = Map.new
nav_tree.nav = {}
nav_tree.nav.source_path = CONTENT_ROOT
nav_tree.nav.nav_level = 0
nav_tree.nav.nav_type = "root"
nav_tree.nav.navigation_yml = (File.exists? (CONTENT_ROOT + "/navigation.yml"))
nav_tree.nav.index_erb = (File.exists? (CONTENT_ROOT + "/index.erb"))							 
			
items = traverse_folders(1, nil, "", "", CONTENT_ROOT)

nav_tree.nav.children = {}
nav_tree.nav.children.count = items.size
nav_tree.nav.children.items = items

File.open("#{ROOT}/tree/cbu_nav_tree.yml", 'w') { |file| file.write(nav_tree.to_yaml) }
File.open("#{ROOT}/tree/cbu_tree.json", 'w') { |file| file.write(nav_tree.to_json) }
...
File.open("#{ROOT}/tree/cbu_nav_tree.yml", 'w') { |file| file.write(nav_tree.to_yaml) }
File.open("#{ROOT}/tree/cbu_nav_tree.json", 'w') { |file| file.write(nav_tree.to_json) }
